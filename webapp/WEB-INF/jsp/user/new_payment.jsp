...
<body>
	<table id="main-container">
		...
		<tr><td class="content center">
			<h3><fmt:message key="newp.header"/> <fmt:formatNumber type="number" minIntegerDigits="5"
			...
			<input name="accountTo" placeholder="<fmt:message key="newp.to"/>" required pattern = "^(\d)+"/><br/>	
			<fmt:message key="newp.hint2"/>	<br/>
			<input name="amount" placeholder="<fmt:message key="newp.am"/>" required pattern = "^(\d)+(\.(\d{1,2}))?"/><br/>			
			<input type="submit" value="<fmt:message key="newp.submit"/>">
			</form>

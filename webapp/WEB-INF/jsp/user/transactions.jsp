...
<body>
	<table id="main-container">
		...
		<tr>
			<td class="content center"><c:if test="${not empty accPayments}">
					<h3>
						<fmt:message key="trns.header" />
							...
							<tr>
								...
								<c:choose>
									<c:when test="${payment.paymentTypeId == 0}">
										...
										<td><fmt:message key="cart.from" /> <fmt:formatNumber
												type="number" minIntegerDigits="5"
												value="${payment.accountIdFrom}" /></td>
										<td><fmt:message key="cart.to" /> <fmt:formatNumber
												type="number" minIntegerDigits="5"
									...
									<c:otherwise>
										...
										<td colspan="2"><fmt:message key="cart.to" /> <fmt:formatNumber
												type="number" minIntegerDigits="5"
												value="${payment.accountIdTo}" /></td>
										<td><fmt:formatNumber type="currency" currencySymbol="$"
												value="${payment.moneyAmount}" /></td>
								...
								<td><fmt:formatDate type="both" dateStyle="short"
										timeStyle="short" value="${payment.date}" /></td>
								<td>
									<form action="controller" method="POST">
										<input type="hidden" name="command"
											value="getTransactionReport" /> <input type="hidden"
											name="paymentId" value="${payment.id}" /> <input
											type="submit" value="<fmt:message key="trns.getbill"/>" />
									</form>
								</td>
							</tr>
						</c:forEach>
					...
					<p>
						...
						<select name="sortby" form="sortform">
							<option value="number"
								<%if (session.getAttribute("paymentsOrder").equals("number")) {%>
								selected <%}%>>No.</option>
							<option value="dateASC"
								<%if (session.getAttribute("paymentsOrder").equals("dateASC")) {%>
								selected <%}%>><fmt:message key="trns.sort.no" /></option>
							<option value="dateDSC"
								<%if (session.getAttribute("paymentsOrder").equals("dateDSC")) {%>
								selected <%}%>><fmt:message key="trns.sort.on" /></option>
						</select>
					</p>
				...
				</c:if> <c:if test="${empty accPayments}">
					<h3>
						<fmt:message key="trns.null" />
					</h3>
					<p>
